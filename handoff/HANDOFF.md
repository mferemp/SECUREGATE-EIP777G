...
### Source ZIP / repo content proof — EXACT delivered file

This proof handoff is delivered SEPARATELY and is NOT packaged inside the source ZIP
(so there is no circular hash and no stale-claim leakage into the artifact).

- final source ZIP: `securegate-eip777g-final.zip`  ·  sha256: `198f0637d476848040b108e367551b8a071ed23d3e0cc1c9c0da98c3535f39a3`
- branch: `securegate/eip-777g-final-build`  ·  commit: `c9437df1f33a826866d922300203e98c16fa5409`
- `git status --short`: empty (see `handoff/proofs/30-git-repo-proof.txt`)
- built from the clean commit above: git-tracked source + Foundry `out/`+`cache/` + the three dotfiles; handoff/proof/deliverable markdown and all `.zip`/`.sha256`/`.b64.txt` excluded

**Acceptance gate on the EXACT delivered source ZIP:**
...
$ python3 verify-dotfiles securegate-eip777g-final.zip
dotfiles verified in exact ZIP
```

**Retrieval.** Primary artifact is the ZIP `securegate-eip777g-final.zip`. Because the studio workspace
export can strip `.zip` and hidden dotfiles, a byte-identical base64 fallback is at
`securegate-eip777g-final.zip.b64.txt`. Reconstruct (skips `#` comment lines):

```
awk '!/^#/' securegate-eip777g-final.zip.b64.txt | base64 -d > securegate-eip777g-final.zip
sha256sum securegate-eip777g-final.zip                 # must equal 198f0637d476848040b108e367551b8a071ed23d3e0cc1c9c0da98c3535f39a3
scripts/with-node24.sh node scripts/verify-zip-contents.cjs securegate-eip777g-final.zip   # [PASS] ZIP content gate satisfied
```
